...
用法：一人在 Gitee（国内直连）或 GitHub 创建一个只勾 `gists` 权限的令牌，**把同一个令牌发给对方，两人各自粘贴登录**，即共用同一份私密云端数据——都能看到、都能写、都能编辑。首次登录各选一个名字和颜色，想法自动署名；支持 ❤️ 回应、互相回复、编辑删除、**每条想法可选字体颜色、可配图片**（自动压缩，点开全屏看），手机可「添加到主屏幕」装成独立 App（PWA，离线可用）。
